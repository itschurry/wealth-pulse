...
장중 자동매매 기본 주기는 `60`초야. `bluechip_core` 우량 후보가 점수 `90` 이상이거나 리서치 점수 `0.75` 이상이면 `watch_only`라도 빈 슬롯이 있을 때 매수 후보로 승격한다. 보유 종목 수가 꽉 찼거나 강한 후보가 현금/노출 제한 때문에 못 사는 상태면 rotation 후보로 올리고, 기존 보유 종목보다 점수 차가 rotation 기준을 넘고 매도 후 사이징이 가능하면 약한 보유 종목을 팔고 새 우량 후보를 산다.
