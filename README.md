...
- 리서치 대상: `KOSPI`
- 자동매매/수동 주문 대상: `KOSPI`
- NASDAQ/NYSE/AMEX/SP100 관련 코드는 나중을 위한 개발 자산으로 남겨두지만, 현재 OpenAI 리서치 타깃과 주문 경로에서는 제외해.
- `--market NASDAQ`, runtime `{"markets":["NASDAQ"]}`, 수동 주문 `market=NASDAQ`은 운용 시장이 아니라 실패하는 게 정상 동작이야.
...
현재 wrapper와 `openai_research_runner.py`는 운용 리서치 시장을 `KOSPI`로 제한해. 미국장 코드는 유지하지만 `--market NASDAQ`은 실패해야 맞아.
...
- 미국 evidence: Nasdaq, SEC 검색 링크. 단, 현재 운용 리서치에는 쓰지 않아.
...
NASDAQ은 현재 주문 운용 시장이 아니야.

```bash
curl -X POST http://127.0.0.1:8001/api/runtime/engine/start \
  -H 'Content-Type: application/json' \
  -d '{"markets":["NASDAQ"],"interval_seconds":300}'
# inactive_auto_trade_market
```
