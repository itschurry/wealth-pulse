...
후보 생성은 한 번에 끝나는 단순 ranking이 아니야. 전략 스캔, 기존 보유, 관심 종목, 리서치 freshness, 실시간 거래대금/등락률/거래량, 뉴스 점수가 섞여.
기본 유니버스는 동적 생성이 아니라 `apps/api/config/universes/kospi100.json` 정적 설정이야. `kospi`, `top_liquidity_200`, `kr_core_bluechips` 규칙은 현재 모두 이 KOSPI100 설정으로 정규화돼.
...
strategy scans + configured universe + held positions + user watchlist + latest research
  ...
  -> KIS realtime quote evidence(current_price/change_pct/volume/trading_value)
  -> market_scanner source tagging
...
- `held`: 이미 보유 중인 종목
- `core`: 상시 감시할 핵심 후보
- `promotion`: 실시간 mover, 뉴스, 거래대금, 등락률, 리서치 점수로 승격된 후보

장중 watchlist refresh는 운용 KOSPI 유니버스의 현재가, 등락률, 거래량, 누적 거래대금을 KIS로 붙인다. 이 시세 기반 스캐너는 `market_scanner` source를 붙이고, `change_pct >= 2.0` 또는 `trading_value >= 500억`이면 `realtime_mover` 후보로 본다. 기본 active slot은 `held + core 20 + promotion 12`이고, promotion은 `realtime_mover`를 먼저 채운다. 강한 mover인데 active slot에 못 들어간 종목은 `promotion_events`에 `missed_mover`로 남긴다.

각 후보 payload에는 `monitor_priority_breakdown`이 들어간다. `components`에는 action, signal_state, rank, score, market_scanner, realtime_mover, news_surge, research_high_score 같은 가산/감산 항목이 그대로 남아. 후보가 왜 active slot에 들어왔는지는 이 값을 먼저 봐.

`/api/monitor/watchlist`는 리서치 runner가 읽는 핵심 API야. 응답 안의 `pending_items`가 OpenAI 리서치 대상이 돼.
`refresh=1`만 주면 새 후보를 계산해서 보여주지만 SQLite 상태는 저장하지 않아. 상태까지 갱신할 때만 `persist=1`을 같이 붙여.

```bash
curl 'http://127.0.0.1:8001/api/monitor/watchlist?market=KOSPI&refresh=1&persist=1&limit=30&mode=missing_or_stale'
```

로컬 helper로 같은 동작을 볼 때는 `--refresh`와 `--persist`를 분리해서 써.

```bash
docker compose exec api python scripts/research_ops.py watchlist --market KOSPI --limit 30 --refresh
docker compose exec api python scripts/research_ops.py watchlist --market KOSPI --limit 30 --refresh --persist
```
...
`bluechip_core` 우량 후보가 점수 `90` 이상이거나 리서치 점수 `0.75` 이상이어도 위 리서치/추세 게이트를 통과해야 매수 후보로 승격한다. `operator_review`에 걸린 후보라도 `entry` 신호, 우량 후보, `buy`/`buy_watch` 리서치, 신선한 리서치, 추세/거래량 조건, 주문 수량을 모두 만족하면 `operator_review_high_momentum_entry`로 신규 매수 후보에 올린다. 리서치 `hold`는 이 승격 대상이 아니다. rotation 매수 후보는 `final_action=review_for_entry`, `agent_primary_buy`, `quant_entry`가 모두 맞아야 한다. `watch_only`, `operator_review`, `agent_hold` 후보는 rotation으로 사지 않는다. 교체 매도는 현재 계좌 기준으로 교체 매수 수량이 이미 1주 이상 나올 때만 실행하고, 매도해서 생길 현금을 가정하지 않는다. rotation 매도는 기본 `min_holding_minutes=30`을 지나야 가능하다. 손절/익절은 이 제한보다 먼저 처리된다.
...
curl 'http://127.0.0.1:8001/api/monitor/watchlist?market=KOSPI&limit=30&refresh=1&persist=1&mode=missing_or_stale'
