...
class ResearchAgentPayloadTests(unittest.TestCase):
    def test_buy_intent_zero_risk_prices_are_rejected(self) -> None:
        payload = {
        ...
        with self.assertRaisesRegex(ValueError, "trade_plan_entry_price_required|invalidation_trigger_stop_loss_invalid|trade_plan_stop_loss_invalid"):
            build_agent_research_ingest_payload(payload)

    def test_buy_intent_requires_explicit_thesis_risk_prices(self) -> None:
        payload = {
            "generated_at": "2026-06-22T09:30:00+09:00",
            "items": [
                {
        ...
        self.assertEqual(item["invalidation_trigger"]["stop_loss"], 94000)
        self.assertEqual(item["trade_plan"]["entry_price"], 100000)
        self.assertEqual(item["trade_plan"]["stop_loss"], 94000)
