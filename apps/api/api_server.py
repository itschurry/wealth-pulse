...
async def api_get(full_path: str, request: Request) -> JSONResponse:
    ...
    return JSONResponse(status_code=status, content=normalize_api_response(path, status, payload))
...
async def _body_dispatch(full_path: str, request: Request, dispatcher) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content=normalize_api_response(path, 400, {"ok": False, "error": "invalid_json"}),
        )
    if not isinstance(body, dict):
        return JSONResponse(
            status_code=400,
            content=normalize_api_response(path, 400, {"ok": False, "error": "json_object_required"}),
        )
    result = await asyncio.to_thread(dispatcher, path, body)
    if result is None:
        return JSONResponse(status_code=404, content={})
    status, response = result
    return JSONResponse(status_code=status, content=normalize_api_response(path, status, response))
